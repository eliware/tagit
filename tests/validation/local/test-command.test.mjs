...
test('builds the shared npm test check and coverage waiver form', () => {
  const fs = {
    ...
    readFileSync: jest.fn(() =>
      JSON.stringify({
        name: 'demo',
        devDependencies: { '@eliware/test': '^3.0.0' },
  ...
  expect(buildTestCheck(fs, { ignoreMonolithLimits: true }).check).toEqual([
    'test',
    ['npm', ['test', '--', '--ignore-monolith-limits']],
  ]);
  expect(buildTestCheck(fs, { ignore100x4: true, ignoreMonolithLimits: true }).check).toEqual([
    'test',
    ['npm', ['test', '--', '--ignore-100x4', '--ignore-monolith-limits']],
  ]);
});
...
test('reports missing package or test script', () => {
  ...
  expect(
    buildTestCheck({ existsSync: jest.fn(() => true), readFileSync: jest.fn(() => JSON.stringify({ scripts: {} })) }),
  ).toEqual({ missing: true, check: null });
});
  ...
  const fs = {
    ...
    readFileSync: () =>
      JSON.stringify({
        name: 'demo',
        devDependencies: { '@eliware/test': '^3.0.0' },
        scripts: { test: 'node bin/eliware-test.mjs' },
      }),
  };
...
test('rejects an unrelated successful test command', () => {
  ...
  expect(
    buildTestCheck({
      existsSync: () => true,
      readFileSync: () =>
        JSON.stringify({ name: 'demo', devDependencies: { '@eliware/test': '^3.0.0' }, scripts: { test: 'jest' } }),
    }),
  ).toEqual({ missing: false, invalid: true, check: null });
});
...
test('rejects a missing or linked shared harness dependency', () => {
  ...
  expect(
    buildTestCheck({
      existsSync: (file) => file !== 'node_modules/@eliware/test/package.json',
      readFileSync: () => packageData,
    }),
...
test('allows the shared harness package to run its own test command', () => {
  const fs = {
    existsSync: () => true,
    readFileSync: () => JSON.stringify({ name: '@eliware/test', scripts: { test: 'node bin/eliware-test.mjs' } }),
  };
  expect(buildTestCheck(fs).check).toEqual(['test', ['npm', ['test']]]);
});
