...
function gitRunner(describe, files, diff) {
  return jest.fn((_executable, args) => (args[0] === 'describe' ? describe : args[1] === '--name-only' ? files : diff));
}
...
test('ignores generated files and classifies minor and major changes', () => {
  ...
  expect(
    suggestVersion(
      fakeFs('1.2.3'),
      gitRunner('v1.2.3', 'src/a.mjs\nsrc/b.mjs\nsrc/c.mjs\nsrc/d.mjs\nsrc/e.mjs', '+'.repeat(250)),
    ),
  ).toMatchObject({ level: 'minor', suggested: '1.3.0' });
  expect(
    suggestVersion(fakeFs('1.2.3'), gitRunner('v1.2.3', 'src/index.mjs', '+ export function changed() {}')),
  ).toMatchObject({ level: 'major', suggested: '2.0.0' });
});
