...
const publicFs = { existsSync: jest.fn(file => file === 'package.json'), readFileSync: jest.fn(() => JSON.stringify({ name: 'demo', private: false })), readdirSync: jest.fn(() => []) };

function runners({ list = [{ databaseId: 1, headSha: 'abc', headBranch: 'v1.0.0' }], details = { status: 'completed', conclusion: 'success', headSha: 'abc', jobs: [{ name: 'ubuntu', status: 'completed', conclusion: 'success' }] }, remote = 'git@github.com:eliware/demo.git', registry = '1.0.0' } = {}) {
...
test('rejects invalid release identity and polling bounds', async () => {
  await expect(verifyRelease(jest.fn(), {}, log, { version: 'next', release: { commitSha: 'abc' } })).rejects.toThrow('must be valid');
...
test('reports ambiguous metadata, invalid remotes, and exhausted inspection', async () => {
  ...
  await expect(verifyRelease(unavailable.execSync, {}, log, { ...validInput, execFile: unavailable.execFile })).rejects.toThrow('inspection failed after');
});
...
test('retries transient CI inspection and reports a non-successful candidate', async () => {
  ...
  transient.execFile.mockImplementation((executable, args, options, callback) => {
    ...
    if (args[1] === 'list') return callback(null, JSON.stringify([{ databaseId: 4, headSha: 'abc', headBranch: 'v1.0.0' }]), '');
...
test('returns links-only evidence without waiting for registry checks', async () => {
  ...
  await expect(verifyRelease(execSync, {}, log, { ...validInput, linksOnly: true, maxPolls: 1, execFile })).resolves.toMatchObject({ linksOnly: true });
  const empty = runners({ details: { status: 'in_progress', conclusion: '', headSha: 'abc', jobs: [] } });
  await expect(verifyRelease(empty.execSync, {}, log, { ...validInput, linksOnly: true, maxPolls: 1, execFile: empty.execFile })).resolves.toMatchObject({ linksOnly: true });
});
...
test('requires Ubuntu, publish, and passing Windows when applicable', async () => {
  ...
  await expect(verifyRelease(noUbuntu.execSync, {}, log, { ...validInput, execFile: noUbuntu.execFile })).rejects.toThrow('Ubuntu');
  const noPublish = runners({ details: { status: 'completed', conclusion: 'success', headSha: 'abc', jobs: [{ name: 'ubuntu', status: 'completed', conclusion: 'success' }] } });
  await expect(verifyRelease(noPublish.execSync, publicFs, log, { ...validInput, execFile: noPublish.execFile })).rejects.toThrow('publish job');
...
test('supports refs/tags, skipped optional jobs, private packages, and GHCR', async () => {
  const tagged = runners({ list: [{ databaseId: 3, headSha: 'abc', headBranch: 'refs/tags/v1.0.0' }], details: { status: 'completed', conclusion: 'success', headSha: 'abc', jobs: [{ name: 'ubuntu', status: 'completed', conclusion: 'success' }, { name: 'optional', status: 'completed', conclusion: 'skipped' }] } });
  await expect(verifyRelease(tagged.execSync, { existsSync: jest.fn(() => false) }, log, { ...validInput, execFile: tagged.execFile })).resolves.toMatchObject({ ci: true, npm: false });
  const digest = `sha256:${'a'.repeat(64)}`;
  const ghcr = runners({ details: { status: 'completed', conclusion: 'success', headSha: 'abc', jobs: [{ name: 'ubuntu', status: 'completed', conclusion: 'success', url: 'https://ci/ubuntu' }, { name: 'publish', status: 'completed', conclusion: 'success', url: 'https://ci/publish' }, { name: 'publish ghcr', status: 'completed', conclusion: 'success', url: 'https://ci/ghcr' }] }, registry: JSON.stringify([{ name: digest, metadata: { container: { tags: ['v1.0.0'] } } }]) });
  const fs = { existsSync: jest.fn(file => file === '.github/workflows'), readdirSync: jest.fn(() => ['ci.yml']), readFileSync: jest.fn(() => 'ghcr.io/eliware/demo') };
  await expect(verifyRelease(ghcr.execSync, fs, log, { ...validInput, release: { ...validInput.release, imageDigest: digest }, execFile: ghcr.execFile, pollMs: 0, sleep: async () => {}, maxPolls: 1 })).resolves.toMatchObject({ ghcr: true });
  await expect(verifyRelease(ghcr.execSync, fs, log, { ...validInput, execFile: ghcr.execFile, pollMs: 0, sleep: async () => {}, maxPolls: 1 })).resolves.toMatchObject({ ghcr: true });
  const publicGhcrFs = {
