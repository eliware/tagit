...
test('selects and returns a completed release workflow', async () => {
  const execFile = jest.fn((_command, args, _options, callback) => callback(null, args[1] === 'list' ? JSON.stringify([{ databaseId: 1, createdAt: '2026-01-01', status: 'completed', conclusion: 'success', headSha: 'abc', headBranch: 'v1.0.0', url: 'url' }]) : JSON.stringify({ databaseId: 1, status: 'completed', conclusion: 'success', headSha: 'abc', jobs: [] }), ''));
...
test('links-only mode returns as soon as the release workflow is discovered', async () => {
  const execFile = jest.fn((_command, args, _options, callback) => callback(null, args[1] === 'list' ? JSON.stringify([{ databaseId: 2, createdAt: '2026-01-01', status: 'completed', conclusion: 'success', headSha: 'abc', headBranch: 'v1.0.0' }]) : JSON.stringify({ databaseId: 2, status: 'completed', conclusion: 'success', headSha: 'abc', jobs: [] }), ''));
...
test('reports failed job details for a completed unsuccessful run', async () => {
  const execFile = jest.fn((_command, args, _options, callback) => callback(null, args[1] === 'list' ? JSON.stringify([{ databaseId: 3, createdAt: '2026-01-01', status: 'completed', conclusion: 'failure', headSha: 'abc', headBranch: 'v1.0.0' }]) : JSON.stringify({ databaseId: 3, status: 'completed', conclusion: 'failure', headSha: 'abc', jobs: [{ name: 'ubuntu', status: 'completed', conclusion: 'failure' }] }), ''));
...
test('reports a detail-level failure when the list and detail records differ', async () => {
  const execFile = jest.fn((_command, args, _options, callback) => callback(null, args[1] === 'list' ? JSON.stringify([{ databaseId: 4, createdAt: '2026-01-01', status: 'completed', conclusion: 'success', headSha: 'abc', headBranch: 'v1.0.0' }]) : JSON.stringify({ databaseId: 4, status: 'completed', conclusion: 'failure', headSha: 'abc', jobs: [{ status: 'completed', conclusion: 'failure' }] }), ''));
...
test('reports a valid detail-level failure', async () => {
  const execFile = jest.fn((_command, args, _options, callback) => callback(null, args[1] === 'list' ? JSON.stringify([{ databaseId: 7, createdAt: '2026-01-01', status: 'completed', conclusion: 'success', headSha: 'abc', headBranch: 'v1.0.0' }]) : JSON.stringify({ databaseId: 7, status: 'completed', conclusion: 'failure', headSha: 'abc', jobs: [{ name: 'ubuntu', status: 'completed', conclusion: 'failure' }] }), ''));
...
test('reports when a failed run has no job records', async () => {
  const execFile = jest.fn((_command, args, _options, callback) => callback(null, args[1] === 'list' ? JSON.stringify([{ databaseId: 5, createdAt: '2026-01-01', status: 'completed', conclusion: 'failure', headSha: 'abc', headBranch: 'v1.0.0' }]) : JSON.stringify({ databaseId: 5, status: 'completed', conclusion: 'failure', headSha: 'abc', jobs: [] }), ''));
...
test('reports unnamed jobs on a failed candidate', async () => {
  const execFile = jest.fn((_command, args, _options, callback) => callback(null, args[1] === 'list' ? JSON.stringify([{ databaseId: 6, createdAt: '2026-01-01', status: 'completed', conclusion: 'failure', headSha: 'abc', headBranch: 'v1.0.0' }]) : JSON.stringify({ databaseId: 6, status: 'completed', conclusion: 'failure', headSha: 'abc', jobs: [{ status: 'completed', conclusion: 'failure' }] }), ''));
  await expect(pollReleaseCi({ execFile, repo: 'eliware/demo', headSha: 'abc', tag: 'v1.0.0', pollMs: 0, maxPolls: 1, sleep: jest.fn(), linksOnly: false, log: { info: jest.fn() } })).rejects.toThrow('malformed job records');
});
test('uses successful details when list metadata is stale', async () => {
  const execFile = jest.fn((_command, args, _options, callback) => callback(null, args[1] === 'list' ? JSON.stringify([{ databaseId: 8, createdAt: '2026-01-01', status: 'completed', conclusion: 'failure', headSha: 'abc', headBranch: 'v1.0.0' }]) : JSON.stringify({ databaseId: 8, status: 'completed', conclusion: 'success', headSha: 'abc', jobs: [] }), ''));
  await expect(pollReleaseCi({ execFile, repo: 'eliware/demo', headSha: 'abc', tag: 'v1.0.0', pollMs: 0, maxPolls: 1, sleep: jest.fn(), linksOnly: false, log: { info: jest.fn() } })).resolves.toMatchObject({ databaseId: 8 });
});
