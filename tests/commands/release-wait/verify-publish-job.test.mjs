...
test('requires a publish job for public packages', () => {
  expect(() => verifyPublishJob([{ name: 'test', conclusion: 'success' }], { applicable: true, packageName: 'demo', isPrivate: false })).toThrow('successful publish job');
...
test('does not accept publish-adjacent job names', () => {
  expect(() => verifyPublishJob([{ name: 'publish image', status: 'completed', conclusion: 'success' }], { applicable: true, packageName: 'demo', isPrivate: false })).toThrow('successful publish job');
});
test('rejects a non-completed publish job', () => {
  expect(() => verifyPublishJob([{ name: 'publish', status: 'in_progress', conclusion: '' }], { applicable: true, packageName: 'demo', isPrivate: false })).toThrow('successful publish job');
});
