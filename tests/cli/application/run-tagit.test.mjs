...
test('release runs the release operation for an explicit version', async () => {
  const gitOperations = jest.fn();
  await runTagit(
    { gitOperations, verifyRelease: noop, runPreflight: noop, log, registerHandlersFn: noop, registerSignalsFn: noop },
...
test('release blocks when package metadata does not match the explicit version', async () => {
  ...
  expect(log.error).toHaveBeenCalledWith(
    expect.objectContaining({ message: expect.stringContaining('package.json version') }),
  );
  expect(gitOperations).not.toHaveBeenCalled();
});
...
test('release-wait verifies the latest tag without release side effects', async () => {
  ...
  await runTagit({ execSync, execFileSync, verifyRelease, log, registerHandlersFn: noop, registerSignalsFn: noop }, [
    'release-wait',
  ]);
  expect(execFileSync).toHaveBeenCalledWith('git', ['rev-list', '-n', '1', `v${packageData.version}`]);
  expect(verifyRelease).toHaveBeenCalledWith(execFileSync, expect.anything(), log, {
...
test('release-wait rejects an explicit version because it always follows the latest tag', async () => {
  ...
  await expect(
    runTagit({ log, exit, registerHandlersFn: noop, registerSignalsFn: noop }, [
      'release-wait',
      '--version',
      packageData.version,
    ]),
...
test('push pushes commits without staging and reports CI links', async () => {
  ...
  await runTagit(
    { output, execSync, execFileSync, reportCiLinks, log, registerHandlersFn: noop, registerSignalsFn: noop },
    ['push'],
  );
...
test('release dry run preflights without changing or publishing anything', async () => {
  ...
  await runTagit(
    ...
    ['release', '--version', packageData.version, '--dry-run'],
  );
  expect(runPreflight).toHaveBeenCalled();
  expect(updateVersionFiles).not.toHaveBeenCalled();
  expect(gitOperations).not.toHaveBeenCalled();
...
test('release rejects a missing version and handles release failures', async () => {
  ...
  await expect(
    runTagit(
      { exit, log: { ...log, error }, suggestVersion: noop, registerHandlersFn: noop, registerSignalsFn: noop },
      ['release'],
    ),
  ).rejects.toThrow();
  expect(exit).toHaveBeenCalledWith(1);
  const failed = jest.fn(() => {
    throw new Error('release operation failed');
  });
  const releaseExit = jest.fn(() => {
    throw new Error('release operation failed');
  });
  await expect(
    runTagit(
  ...
  ).rejects.toThrow('release operation failed');
  expect(exit).toHaveBeenCalledWith(1);
});
