...
test('reads explicit inapplicable path reasons', () => {
  const fs = {
    existsSync: () => true,
    readFileSync: () => JSON.stringify({ inapplicable: { 'examples/': 'CLI-only package has no runnable examples.' } }),
  };
...
test('rejects malformed exception files', () => {
  ...
  expect(() =>
    readRepositoryExceptions({
      existsSync: () => true,
      readFileSync: () => JSON.stringify({ inapplicable: { 'docs/': '' } }),
    }),
