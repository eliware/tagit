...
test('reports a detached worktree explicitly', () => {
  const failures = [];
  validateRepository((command, args) => args[0] === 'branch' ? '' : '', { existsSync: () => true, readFileSync: file => file === '.tagit-exceptions.json' ? '{"inapplicable":{}}' : JSON.stringify({ name: 'x', version: '1', description: 'x', license: 'MIT' }) }, failures);
  expect(failures.join('\n')).toContain('detached');
});
