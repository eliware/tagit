...
test('detects GHCR declarations in workflow files', () => {
  const fs = { existsSync: () => true, readdirSync: () => ['ci.yml', 'notes.txt', 'other.yaml'], readFileSync: file => file.endsWith('ci.yml') ? 'image: ghcr.io/eliware/demo' : 'name: test' };
...
test('does not infer GHCR publication from missing or unrelated workflows', () => {
  ...
  expect(publishesGhcr({ existsSync: () => true, readdirSync: () => ['ci.yml'], readFileSync: () => 'npm publish' })).toBe(false);
});
