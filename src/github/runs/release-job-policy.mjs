export function verifyReleaseJobs(jobs) {
  ...
  const failed = jobs.filter(
    (job) => job.status !== 'completed' || !['success', 'skipped', 'neutral'].includes(job.conclusion),
  );
  if (failed.length)
    throw new Error(`Release CI failed: ${failed.map((job) => `${job.name}: ${job.conclusion}`).join('; ')}`);
  if (!jobs.some((job) => successful(job) && /ubuntu/i.test(job.name)))
