export function selectReleaseRun(runs, headSha, tag) {
  ...
  if (
    ...
    throw new Error(`Release CI evidence is malformed for ${tag}: run identity is incomplete.`);
  if (runs.some((item) => item.createdAt !== undefined && !Number.isFinite(new Date(item.createdAt).getTime())))
    throw new Error(`Release CI evidence is malformed for ${tag}: creation timestamp is invalid.`);
  const eligible = runs
  ...
  if (eligible.length > 1 && eligible.some((item) => !item.createdAt) && eligible.some((item) => item.createdAt))
    throw new Error(
      `Release CI evidence is ambiguous because eligible runs are missing creation timestamps for ${tag}.`,
    );
  return eligible[0];
}
