...
export function readRepositoryExceptions(fs) {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value) || !value.inapplicable || typeof value.inapplicable !== 'object') {
  ...
  for (const [path, reason] of Object.entries(value.inapplicable)) {
    if (!path || typeof reason !== 'string' || !reason.trim()) throw new Error(`${EXCEPTIONS_FILE} requires a non-empty reason for ${path}.`);
  }
