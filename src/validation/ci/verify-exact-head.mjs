...
export function verifyLatestCi(execFileSync, log, { headSha, repository = null, waitForCompletion = true } = {}, pollAttempt = 0, selectedRun = null) {
  ...
  try {
    const args = selectedRun
      ...
      : ['run', 'list', '--commit', headSha, ...repoArg, '--limit', '20', '--json', 'databaseId,status,conclusion,headSha,url'];
    const parsed = JSON.parse(gh(args));
    runs = validateRunRecords(selectedRun && !Array.isArray(parsed) ? [{ ...selectedRun, ...parsed }] : parsed, headSha);
  }
