...
export function verifyCompletedRun(execFileSync, log, run, headSha) {
  ...
  if (!data || typeof data.status !== 'string' || typeof data.conclusion !== 'string') throw new Error(`GitHub Actions run ${run.databaseId} returned malformed completion metadata.`);
