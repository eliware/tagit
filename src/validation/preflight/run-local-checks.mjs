...
export function runLocalChecks(execFileSync, fs, { ignore100x4 = false, ignoreMonolithLimits = false, timeoutMs = 120000 } = {}) {
  const failures = [];
  const testCheck = buildTestCheck(fs, { ignore100x4: coverageWaiverPolicy(ignore100x4).ignored, ignoreMonolithLimits });
  ...
  try { const [command, commandArgs] = processCommand(executable, args); execFileSync(command, commandArgs, processOptions(command, timeoutMs)); results.test = { passed: true }; }
  catch (error) { results.test = { passed: false }; failures.push(failureMessage('test', error, `${error.stdout ?? ''}\n${error.stderr ?? ''}`)); }
