export function redactSecrets(output) {
  ...
  return String(output)
    .replace(
      /((?:token|password|secret|authorization|api[_-]?key|access[_-]?key|secret[_-]?key)\s*[:=]\s*)(["'])(.*?)\2/gi,
      '$1[REDACTED]',
    )
    ...
    .replace(/\bnpm_[A-Za-z0-9_-]{20,}\b/g, '[REDACTED]')
    .replace(/-----BEGIN [A-Z ]*PRIVATE KEY-----[\s\S]*?-----END [A-Z ]*PRIVATE KEY-----/g, '[REDACTED PRIVATE KEY]')
    .replace(
      /(token|password|secret|authorization|npm_[^=\s]*|ghp_[A-Za-z0-9_-]+)\s*[:=]\s*["']?[^\s"']+["']?/gi,
      '$1=[REDACTED]',
    )
